...
export async function listAccounts(accessToken) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      detail = ` — ${JSON.stringify(errJson.error || errJson)}`;
    } catch (_) {
      try {
        detail = ` — ${await response.text()}`;
      } catch (__) {}
    }
    throw new Error(`GBP accounts error: ${response.status} ${response.statusText}${detail}`);
  }
...
export async function listLocations(accountName, accessToken) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`GBP locations error: ${response.status} ${response.statusText}${detail}`);
  }
  const data = await response.json();
...
export async function publishPost({ description, imagePath, locationName, accessToken }) {
  ...
  const isVideo = ['mp4', 'mov', 'webm', 'ogg', 'mkv', 'm4v'].includes(ext);
  ...
  // Step 2: Upload media bytes (dynamically using correct content-type)
